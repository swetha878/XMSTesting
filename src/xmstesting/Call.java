 ...
 * @author ssatyana
 */
public class Call extends Observable implements SipCall {
        ...
        switch (request.getMethod()) {
            case Request.INVITE:
                if (isInvite) {
                    ...
                    Event eve = createRequestEvent(request, EventType.INCOMING);
                    setValue(eve);
                    isInvite = true;
                }
                break;
            case Request.OPTIONS:
                ...
                break;
            case Request.INFO:
                Event eve = createRequestEvent(request, EventType.INFOREQUEST);
                setValue(eve);
                break;
            case Request.ACK:
                Event event = createRequestEvent(request, EventType.CONNECTED);
                setValue(event);
                break;
            case Request.CANCEL:
                break;
            case Request.BYE:
                Event e = createRequestEvent(request, EventType.DISCONNECTED);
                setValue(e);
                break;
        }
    }
                ...
                switch (cSeq.getMethod()) {
                    case Request.INVITE:
                        ...
                        if (pendingCallList.contains(this)) {
                            ...
                            Event eve = createResponseEvent(response, EventType.CONNECTING);
                            setValue(eve);
                        }
                        break;
                    case Request.INFO:
    ...
    public void makeCall(String dest) {
        ...
        String toUserId = params[0];
        String toAdr = params[1];
        createInviteRequest(toUserId, toAdr);
    }
    ...
    public void acceptCall() {
        this.createRingingResponse(this.getInviteRequest());
    }

    @Override
    public void answerCall() {
        createInviteOk(this.getInviteRequest());
    }
    ...
    @Override
    public void play(String msml) {
        createInfoRequest(msml);
    }
